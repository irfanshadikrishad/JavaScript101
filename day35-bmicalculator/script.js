...
metr.addEventListener('click', ()=> {
    ...
    if (indo == 1){
        document.getElementById('feet').placeholder = 'cm (eg:160)';
    } else if (indo==0){
        document.getElementById('feet').placeholder = 'feet (eg:5.5)';
    }
})
//
function calculate(){
    ...
    if(index==0){
        ...
        if(fres < 18 || fres > 30){
            document.getElementsByClassName('bmi')[0].style.backgroundColor = 'rgba(206, 6, 6, 0.87)';
        } else if (fres > 18.5 && fres < 25){
            document.getElementsByClassName('bmi')[0].style.backgroundColor = 'green';
        } else if (fres >25 && fres <30){
            document.getElementsByClassName('bmi')[0].style.backgroundColor = 'tomato';
        }
    } else if(index==1){
        const wei = document.getElementById('weight').value;
        const hei = document.getElementById('feet').value;
