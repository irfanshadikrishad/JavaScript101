...
window.addEventListener("keydown", (e) => {
  ...
  if (e.code === "Space" && document.activeElement !== q) {
    e.preventDefault();
    toggleSpeak();
  }
});
...
async function doSuggest(term) {
  ...
  try {
    const url =
      "https://en.wikipedia.org/w/api.php?action=opensearch&format=json&limit=8&origin=*" +
      "&search=" +
...
searchBtn.addEventListener("click", () => {
  if (q.value.trim()) loadArticle(q.value.trim());
});
...
async function loadArticle(title) {
  try {
    ...
    subtitle.textContent =
      lastArticle.extract.slice(0, 150) +
      (lastArticle.extract.length > 150 ? "..." : "");
    thumb.innerHTML = lastArticle.image
      ? `<img src="${lastArticle.image}" style="width:100%;height:100%;object-fit:cover"/>`
      : "No image";
    contentEl.innerHTML = `<p>${lastArticle.extract}</p>`;

    // compute keywords & snippets
    const sentences = lastArticle.extract
      .split(/(?<=[.!?])\s+/)
      .filter(Boolean);
...
$("#compareBtn").addEventListener("click", () => {
  ...
  window.open(
    "https://en.wikipedia.org/wiki/" + encodeURIComponent(other),
    "_blank",
  );
});
